...
doc.add_paragraph('cd memo', style='No Spacing')
...
doc.add_paragraph('LLM_API_KEY=sk-你的key', style='No Spacing')
doc.add_paragraph('LLM_BASE_URL=https://api.deepseek.com/v1', style='No Spacing')
...
doc.add_paragraph(
    '{"mcpServers": {"memo": {"command": "python", "args": ["<项目路径>/scripts/run_mcp.py"]}}}',
    style='No Spacing'
)

doc.add_heading('6.6 启动后台服务', level=2)
doc.add_paragraph('双击 start_all.bat 一键启动看板 + 自动同步。', style='List Bullet')
doc.add_paragraph('双击 stop_all.bat 停止所有后台服务。', style='List Bullet')
...
faq = [
    ...
    ('Q: 数据安全吗？', 'A: 全部数据存储在本地的单个 SQLite 文件中（memo/data/memo.db）。无云服务、无遥测、无第三方传输。LLM 提取时只传当前对话片段（不传全量历史）。'),
    ...
    ('Q: 怎么备份？', 'A: 复制 memo/data/memo.db 文件即可。SQLite 单文件，随时复制。'),
]
for q, a in faq:
    p = doc.add_paragraph()
    run = p.add_run(q + ' ')
...
# ── 保存 ──
output_path = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "docs", "Memo-项目说明文档.docx")
